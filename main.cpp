  ...
  while (window.isOpen()) {
    ...
    while (window.pollEvent(event)) {
      ...
      if (event.type == sf::Event::KeyPressed) {
        ...
        // часть для обработки смещения с помощью стрелочек
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Up)) {
          --VectorShift::y;
          text_field_1.CountPoints();
          line_x.setPosition(sf::Vector2f(0, window.getSize().y / 2.f + VectorShift::y * Screen::scale));
          line_y.setPosition(sf::Vector2f(window.getSize().x / 2 + VectorShift::x * Screen::scale, 0));
          std::cout << VectorShift::x << ' ' << VectorShift::y << '\n';
        }
      ...
      if ((event.type == sf::Event::MouseWheelScrolled) && text_field_1.is_graph_) {
        if (event.mouseWheelScroll.wheel == sf::Mouse::VerticalWheel) {
          ...
          if (event.mouseWheelScroll.delta == -1) {
            if (Screen::scale > 0.0625) {
              Screen::scale /= 2;
              Screen::delta = std::max(0.125, 1 / Screen::scale);
              Screen::delta *= 2;
              line_x.setPosition(sf::Vector2f(0, window.getSize().y / 2.f + VectorShift::y * Screen::scale));
              line_y.setPosition(sf::Vector2f(window.getSize().x / 2 + VectorShift::x * Screen::scale, 0));
              text_field_1.CountPoints();
        ...
        std::cout << Screen::scale << '\n';
      }

      // отрисовка 
      window.clear();

      WriteCursorCoordinates(window, Screen::scale);

      text_field_1.HandleInput(event);
      window.draw(line_x);
      window.draw(line_y);
      DrawNetwork(network_flag, window, Screen::scale);
      window.display();
    }
  }
  return 0;
